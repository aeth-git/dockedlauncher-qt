...
def _make_backup(tmp_path, *triples):
    """Build a minimal synthetic iTunes backup and return an opened BackupSource.

    Each element of *triples is a (domain, rel_path, content_bytes) tuple.
    """
    ...
    for domain, rel, content in triples:
        ...
        (tmp_path / fid[:2]).mkdir(parents=True, exist_ok=True)
